...
def plot_nodes(nodes, segs):
    for i in range(len(nodes)):
        ...
        if n.ty == 'o':
            ...
            #print '3 4 poly fill'
            print '0 -3 moveto 0 3 lineto stroke'
        elif n.ty in ('v', '{', '}'):
...
def run_path(path, show_iter = False, n = 10, xo = 36, yo = 550, xscale = .25, yscale = 2000, pl_nodes = True, pl_path = True):
    segs, nodes = setup_path(path)
    print '.5 setlinewidth'
    for i in range(n):
        if i == n - 1 and pl_path:

...
def normal_distance(s_extremum, s_other, curve):
    if s_extremum is None or s_other is None: return None
    x_e, y_e = curve.xy(s_extremum)
    th = curve.th(s_extremum)
    x_o, y_o = curve.xy(s_other)
    return hypot((x_o - x_e) * sin(th), (y_o - y_e) * cos(th))

def merge_breaks(breaks, extrema, curve, thresh = 2):
    print '%breaks', breaks
	...
	last_break = None
    while 1:
	if extrema_ix < len(extrema):
	    s_e = extrema[extrema_ix]
	    if breaks_ix < len(breaks):
		next_break = breaks[breaks_ix]
	    elif len(breaks):
		next_break = breaks[0]
	    else:
		next_break = None
	    do_extremum = (breaks_ix == len(breaks) or next_break > s_e)
	else:
	    if breaks_ix < len(breaks):
		do_extremum = False
	    else:
		return result
	if do_extremum:
	    d1 = normal_distance(s_e, last_break, curve)
	    d2 = normal_distance(s_e, next_break, curve)
	    if (d1 is None or d1 > thresh) and (d2 is None or d2 > thresh):
		last_break = s_e
		result.append(s_e)
	    extrema_ix += 1
	else:
	    last_break = breaks[breaks_ix]
	    result.append(last_break)
	    breaks_ix += 1

def pcorn_curve_to_bzs(curve, optim = 3, thresh = 1e-3):
    extrema = curve.find_extrema()
    extrema.sort()
    extrema = merge_breaks(curve.find_breaks(), extrema, curve)
    print '%', extrema
    result = []

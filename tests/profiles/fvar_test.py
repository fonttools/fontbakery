...
from fontbakery.codetesting import assert_PASS, assert_results_contain, CheckTester
...
def test_check_varfont_regular_wght_coord():
    ...
    assert_PASS(check(ttFont), "with a good Regular:wght coordinate...")
    ...
    assert_results_contain(
        check(ttFont), FAIL, "not-400", "with a bad Regular:wght coordinate (500)..."
    )
...
def test_check_varfont_regular_wdth_coord():
    ...
    ttFont = TTFont("data/test/cabinvfbeta/CabinVFBeta.ttf")
    assert_PASS(check(ttFont), "with a good Regular:wdth coordinate...")

    # We then ensure the check detects it when we
    # introduce the problem by setting a bad value:
    ttFont["fvar"].instances[0].coordinates["wdth"] = 0
    assert_results_contain(
        check(ttFont), FAIL, "not-100", "with a bad Regular:wdth coordinate (0)..."
    )
...
def test_check_varfont_regular_slnt_coord():
    ...
    assert_results_contain(
        check(ttFont), FAIL, "non-zero", "with a bad Regular:slnt coordinate (12)..."
    )

    # We then fix the Regular:slnt coordinate value in order to PASS:
    assert_PASS(
        check(ttFont, {"regular_slnt_coord": 0}),
...
def test_check_varfont_regular_ital_coord():
    ...
    # And with this the check must detect the problem:
    assert_results_contain(
        check(ttFont), FAIL, "non-zero", "with a bad Regular:ital coordinate (123)..."
    )

    # but with zero it must PASS the check:
    assert_PASS(
        check(ttFont, {"regular_ital_coord": 0}),
...
def test_check_varfont_regular_opsz_coord():
    ...
    assert_results_contain(
        check(ttFont), WARN, "out-of-range", "with a bad Regular:opsz coordinate (9)..."
    )
...
def test_check_varfont_bold_wght_coord():
    ...
    assert_results_contain(
        check(ttFont), FAIL, "not-700", "with a bad Bold:wght coordinage (600)..."
    )
...
def test_check_varfont_wght_valid_range():
    ...
    assert_results_contain(check(ttFont), FAIL, "out-of-range", "with wght=0...")

    # And yet another bad value:
    ttFont["fvar"].instances[0].coordinates["wght"] = 1001
    assert_results_contain(check(ttFont), FAIL, "out-of-range", "with wght=1001...")

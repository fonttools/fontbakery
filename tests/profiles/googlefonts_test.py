...
def test_check_canonical_filename():
    ...
    assert_results_contain(
        check(TEST_FILE("mutatorsans-vf/MutatorSans-VF.ttf")),
        FAIL,
        "unknown-name",
...
def test_check_description_max_length():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/description/max_length"
    )
    ...
    good_length = "a" * 1999
    assert_PASS(check(font, {"description": good_length}), "with 1999-byte buffer...")
...
def test_check_name_rfn():
    ...
    assert_results_contain(
        check(ttFont), FAIL, "rfn", 'with "Reserved Font Name" on a name table entry...'
    )
...
def test_check_family_has_license():
    ...
    assert_results_contain(
        check([], {"licenses": detected_licenses}),
        FAIL,
        "multiple",
    ...
    detected_licenses.pop(-1)  # hack
    assert_PASS(
        check([], {"licenses": detected_licenses}), "with a single OFL license..."
    )

    detected_licenses = licenses(portable_path("data/test/028/pass_apache"))
    detected_licenses.pop(-1)  # hack
    assert_PASS(
        check([], {"licenses": detected_licenses}), "with a single Apache license..."
    )
...
def test_check_metadata_listed_on_gfonts():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/metadata/listed_on_gfonts"
    )
    ...
    assert_results_contain(
        ...
        WARN,
        "not-found",
        f'with "{font}", from a family that\'s' f" not listed on GFonts...",
    )
    ...
    assert_PASS(
        check(font),
        f'with "{font}", from a family that is' f" listed on Google Fonts API...",
    )

    font = TEST_FILE("abeezee/ABeeZee-Regular.ttf")
    # This is to ensure the code handles well camel-cased familynames.
    assert_PASS(check(font), f'with "{font}", listed and with a camel-cased name...')
...
def test_check_metadata_filenames():
    ...
    assert_PASS(check(montserrat_fonts), "with matching list of font files...")
...
def test_check_metadata_fontname_not_camel_cased():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/metadata/fontname_not_camel_cased"
    )
    ...
    for good_name in ["VT323", "PT Sans", "Amatic SC"]:
        md.name = good_name
        assert_PASS(
            check(font, {"font_metadata": md}),
...
def test_check_italic_angle():
    ...
    check = CheckTester(googlefonts_profile, "com.google.fonts/check/italic_angle")
...
def test_check_mac_style():
    ...
    check = CheckTester(googlefonts_profile, "com.google.fonts/check/mac_style")
    from fontbakery.constants import MacStyle
...
def test_check_name_familyname():
    ...
    check = CheckTester(googlefonts_profile, "com.google.fonts/check/name/familyname")
    ...
    test_cases = [
        ...
        (
            PASS,
            "ok",
            TEST_FILE("overpassmono/OverpassMono-Bold.ttf"),
            "Overpass Mono",
            "Overpass Mono",
        ),
        (
...
def test_check_name_subfamilyname():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/name/subfamilyname"
    )
...
def test_check_name_fullfontname():
    ...
    check = CheckTester(googlefonts_profile, "com.google.fonts/check/name/fullfontname")

    # Our reference Cabin Regular is known to be good
    ttFont = TTFont(TEST_FILE("cabin/Cabin-Regular.ttf"))
    assert_PASS(check(ttFont), "with a good Regular font...")
    ...
    assert_PASS(check(ttFont), "with a good Bold Italic font...")

    # And here we test the FAIL codepath:
    for index, name in enumerate(ttFont["name"].names):
        if name.nameID == NameID.FULL_FONT_NAME:
            backup = name.string
            ttFont["name"].names[index].string = "MAKE IT FAIL".encode(
                name.getEncoding()
            )
            assert_results_contain(
                check(ttFont), FAIL, "bad-entry", "with a bad FULL_FONT_NAME entry..."
            )
            # restore it:
            ttFont["name"].names[index].string = backup
    ...
    assert_PASS(check(ttFont), "with a good camel-cased fontname...")
...
def test_check_name_typographicfamilyname():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/name/typographicfamilyname"
    )
    ...
    ttFont["name"].names[
        5
    ].nameID = NameID.TYPOGRAPHIC_FAMILY_NAME  # 5 is arbitrary here
    assert_results_contain(
        check(ttFont),
        FAIL,
        "ribbi",
...
def test_check_name_typographicsubfamilyname():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/name/typographicsubfamilyname"
    )
    ...
    assert_results_contain(check(ttFont), FAIL, "bad-win-name")
    assert_results_contain(check(ttFont), FAIL, "bad-mac-name")
    ...
    ttFont = TTFont(TEST_FILE(NON_RIBBI))
    win_name = ttFont["name"].getName(
        NameID.TYPOGRAPHIC_SUBFAMILY_NAME,
        PlatformID.WINDOWS,
        WindowsEncodingID.UNICODE_BMP,
        WindowsLanguageID.ENGLISH_USA,
    )
    mac_name = ttFont["name"].getName(
        NameID.TYPOGRAPHIC_SUBFAMILY_NAME,
        PlatformID.MACINTOSH,
        MacintoshEncodingID.ROMAN,
        MacintoshLanguageID.ENGLISH,
...
def test_check_varfont_generate_static():
    ...
    del ttFont["fvar"]
    assert_results_contain(check(ttFont), FAIL, "varlib-mutator")
...
def test_check_fvar_name_entries():
    ...
    check = CheckTester(googlefonts_profile, "com.google.fonts/check/fvar_name_entries")
    ...
    assert_results_contain(check(ttFont), FAIL, "missing-name", "with a bad font...")
...
def test_check_varfont_has_instances():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/varfont_has_instances"
    )
    ...
    assert_results_contain(
        check(ttFont), FAIL, "lacks-named-instances", "with a bad font..."
    )
...
def test_check_varfont_weight_instances():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/varfont_weight_instances"
    )
    ...
    # So it must FAIL the check:
    assert_results_contain(check(ttFont), FAIL, "bad-coordinate", "with a bad font...")

    # Let's then change the weight coordinates to make it PASS the check:
    for i, instance in enumerate(ttFont["fvar"].instances):
        ttFont["fvar"].instances[i].coordinates["wght"] -= (
            instance.coordinates["wght"] % 100
        )
...
def test_check_ligature_carets():
    ...
    assert_results_contain(check(ttFont), WARN, "lacks-caret-pos", "with a bad font...")

    # And FamilySans Regular is also bad
    ttFont = TTFont("data/test/familysans/FamilySans-Regular.ttf")
    assert_results_contain(check(ttFont), WARN, "GDEF-missing", "with a bad font...")
...
def test_check_kerning_for_non_ligated_sequences():
    ...
    ttFont = TTFont(TEST_FILE("merriweather/Merriweather-Regular.ttf"))
    assert_results_contain(check(ttFont), WARN, "lacks-kern-info", "with a bad font...")
...
def test_check_cjk_not_enough_glyphs():
    ...
    assert_PASS(check(ttFont), "for Montserrat")
    ...
    cmap.cmap[0x4E00] = "A"
    assert_results_contain(
        check(ttFont), WARN, "cjk-not-enough-glyphs", "There is only 1 CJK glyphs"
    )
    # Add second character of the CJK unified Ideographs
    cmap.cmap[0x4E01] = "B"
    assert_results_contain(
        check(ttFont), WARN, "cjk-not-enough-glyphs", "There are only 2 CJK glyphs"
    )


def test_check_varfont_instance_coordinates(vf_ttFont):
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/varfont_instance_coordinates"
    )
...
def test_check_varfont_instance_names(vf_ttFont):
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/varfont_instance_names"
    )
...
def test_check_varfont_unsupported_axes():
    ...
    ttFont["fvar"].axes.append(new_axis)
    assert_results_contain(check(ttFont), FAIL, "unsupported-slnt")
...
def test_check_gfaxisregistry_bounds():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/metadata/gf-axisregistry_bounds"
    )
...
def test_check_gf_axisregistry_fvar_axis_defaults():
    """Validate METADATA.pb axes tags are defined in gf-axisregistry."""
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/gf-axisregistry/fvar_axis_defaults"
    )
...
def test_check_STAT_gf_axisregistry():
    ...
    check = CheckTester(
        googlefonts_profile, "com.google.fonts/check/STAT/gf-axisregistry"
    )
...
def test_check_STAT_axis_order():
    ...
    assert_results_contain(check(fonts), ERROR, "bad-font")
...
def test_check_mandatory_avar_table():
    ...
    del ttFont["avar"]
    assert_results_contain(check(ttFont), FAIL, "missing-avar")
...
def test_check_description_family_update():
    ...
    assert_results_contain(
        check(font, {"description": desc}), FAIL, "description-not-updated"
    )
...
def test_check_metadata_can_render_samples():
    ...
    assert_results_contain(check(metadata_file), INFO, "no-samples")
    ...
    assert_results_contain(
        check(metadata_file, {"family_metadata": md}), FAIL, "sample-glyphs"
    )
...
def test_check_metadata_unsupported_subsets():
    ...
    assert_results_contain(check(font, {"family_metadata": md}), WARN, "unknown-subset")

    del md.subsets[:]
    md.subsets.extend(["cyrillic"])
    assert_results_contain(
        check(font, {"family_metadata": md}), WARN, "unsupported-subset"
    )

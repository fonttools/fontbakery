...
def test_check_ufolint(empty_ufo_font):
    ...
    assert_PASS(check(ufo_path), "with an empty UFO.")

    os.remove(os.path.join(ufo_path, "metainfo.plist"))
    assert_results_contain(check(ufo_path), FAIL, "ufolint-fail", "with maimed UFO.")


def test_check_required_fields(empty_ufo_font):
    check = CheckTester(ufo_sources_profile, "com.daltonmaag/check/ufo-required-fields")

    ufo, _ = empty_ufo_font

    assert_results_contain(
        check(ufo), FAIL, "missing-required-fields", "with an empty UFO."
    )
    ...
    assert_PASS(check(ufo), "with an almost empty UFO.")


def test_check_recommended_fields(empty_ufo_font):
    check = CheckTester(
        ufo_sources_profile, "com.daltonmaag/check/ufo-recommended-fields"
    )

    ufo, _ = empty_ufo_font

    assert_results_contain(
        check(ufo), WARN, "missing-recommended-fields", "with an empty UFO."
    )
...
def test_check_unnecessary_fields(empty_ufo_font):
    check = CheckTester(
        ufo_sources_profile, "com.daltonmaag/check/ufo-unnecessary-fields"
    )

    ufo, _ = empty_ufo_font

    assert_PASS(check(ufo), "with an empty UFO.")
    ...
    assert_results_contain(
        check(ufo), WARN, "unnecessary-fields", "with unnecessary fields in UFO."
    )

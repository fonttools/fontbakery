...
def test_check_ufolint(empty_ufo_font):
    ...
    assert_PASS(check({"ufo": ufo_path}),
                'with an empty UFO.')

    os.remove(os.path.join(ufo_path, "metainfo.plist"))
    assert_results_contain(check({"ufo": ufo_path}),
                           FAIL, "ufolint-fail",
                           'with maimed UFO.')
...
def test_check_required_fields(empty_ufo_font):
    ...
    assert_results_contain(check({}, {"ufo_font": ufo}),
    ...
    assert_PASS(check({}, {"ufo_font": ufo}),
                'with an almost empty UFO.')


def test_check_recommended_fields(empty_ufo_font):
    check = CheckTester(ufo_sources_profile, "com.daltonmaag/check/ufo-recommended-fields")
    ufo, _ = empty_ufo_font
...
def test_check_unnecessary_fields(empty_ufo_font):
    check = CheckTester(ufo_sources_profile, "com.daltonmaag/check/ufo-unnecessary-fields")
    ufo, _ = empty_ufo_font

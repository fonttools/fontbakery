...
class PyFontBakeryObject(PyObject):
    ...
    def handle_signature(self, sig, signode):
        ...
        try:
            res = (
                self._handle_signature(cid, lineno, sig, signode)
                if cid is not None
...
def setup(app):
    ...
    app.add_directive_to_domain(
        "py", "fontbakerycallable", PyFontBakeryObject, override=False
    )
    app.add_directive_to_domain(
        "py", "fontbakerycheck", PyFontBakeryObject, override=False
    )
    app.add_directive_to_domain(
...
def _process_docstring(app, what, name, obj, options, lines):
    ...
    * ``napoleon_numpy_docstring`` -- parse NumPy style docstrings
    ...
    if hasattr(obj, "proposal") and obj.proposal:
        ...
        if legacy_name:
            lines.append(
                f"**Legacy check** originally simply called '{legacy_name[0]}'."

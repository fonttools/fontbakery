...
def com_google_fonts_check_canonical_filename(font):
    ...
    from .shared_conditions import is_variable_font, variable_font_filename
...
def com_google_fonts_check_description_broken_links(description_html):
    ...
    for a_href in doc.iterfind(".//a[@href]"):
        ...
        if link.startswith("mailto:") and "@" in link and "." in link.split("@")[1]:
            yield INFO, Message("email", f"Found an email address: {link}")
            continue
...
def com_google_fonts_check_description_max_length(description):
    ...
    if len(description) >= 2000:
        yield FAIL, Message(
            "too-long",
            "DESCRIPTION.en_us.html must" " have size smaller than 2000 bytes.",
        )
    else:
...
@check(
    id="com.google.fonts/check/metadata/broken_links", conditions=["family_metadata"]
)
def com_google_fonts_check_metadata_broken_links(family_metadata):
    ...
    for font_metadata in family_metadata.fonts:
        ...
        if "mailto:" in copyright:
            ...
            yield INFO, Message("email", f"Found an email address: {copyright}")
            continue
...
def com_google_fonts_check_metadata_category(family_metadata):
    ...
    if family_metadata.category not in [
        "MONOSPACE",
        "SANS_SERIF",
        "SERIF",
        "DISPLAY",
...
def com_google_fonts_check_usweightclass(ttFont, expected_style):
    ...
    if is_variable_font(ttFont):
        if not has_expected_value:
            failed = True
            yield FAIL, Message("bad-value", fail_message.format(value, expected_value))
    ...
    # for static otfs, Thin must be 250 and ExtraLight must be 275
        ...
        if is_cff(ttFont) and value != 250:
            failed = True
            yield FAIL, Message("bad-value", fail_message.format(value, 250))
        ...
        if is_cff(ttFont) and value != 275:
            failed = True
            yield FAIL, Message("bad-value", fail_message.format(value, 275))
...
@check(
    ...
    """,
    conditions=["font_metadata", "not camelcased_familyname_exception"],
...
def com_google_fonts_check_name_familyname(ttFont, style, familyname_with_spaces):
        ...
        if name.nameID == NameID.FONT_FAMILY_NAME:
            ...
            if name.platformID == PlatformID.MACINTOSH:
                ...
                if style in ["Regular", "Italic", "Bold", "Bold Italic"]:
                    ...
                    expected_value = " ".join(
                        [familyname_with_spaces, only_weight]
                    ).strip()
            else:
                failed = True
                yield FAIL, Message(
                    "lacks-name",
                    f"Font should not have a " f"{name_entry_id(name)} entry!",
                )
                continue

            string = name.string.decode(name.getEncoding()).strip()

            if camelcased_familyname_exception(
                string
            ) and string == expected_value.replace(" ", ""):
                continue
    ...
    if not failed:
        yield PASS, Message("ok", "FONT_FAMILY_NAME entries are all good.")
...
def com_google_fonts_check_name_subfamilyname(ttFont, expected_style):
    ...
    win_name = nametable.getName(
        NameID.FONT_SUBFAMILY_NAME,
        PlatformID.WINDOWS,
        WindowsEncodingID.UNICODE_BMP,
        WindowsLanguageID.ENGLISH_USA,
    )
    mac_name = nametable.getName(
        NameID.FONT_SUBFAMILY_NAME,
...
@check(
    ...
    """,
    conditions=["style_with_spaces", "familyname_with_spaces"],
    proposal="legacy:check/159",
)
def com_google_fonts_check_name_fullfontname(
    ttFont, style_with_spaces, familyname_with_spaces
):
    ...
    for name in ttFont["name"].names:
        if name.nameID == NameID.FULL_FONT_NAME:
            ...
            expected_value = "{} {}".format(familyname, style_with_spaces)
            string = name.string.decode(name.getEncoding()).strip()
...
def com_google_fonts_check_name_typographicfamilyname(
    ttFont, style, familyname_with_spaces
):
...
def com_google_fonts_check_varfont_has_instances(ttFont):
    ...
    if len(ttFont["fvar"].instances):
        ...
        yield FAIL, Message(
            "lacks-named-instances",
            "This variable font lacks" " named instances on the fvar table.",
        )
...
def com_google_fonts_check_kerning_for_non_ligated_sequences(
    ttFont, ligatures, has_kerning_info
):
...
@check(
    ...
    conditions=["family_directory", "gfonts_repo_structure", "is_variable_font"],
    rationale="""
...
def com_google_fonts_check_varfont_instance_coordinates(ttFont):
    ...
    for instance in ttFont["fvar"].instances:
        ...
        for axis in instance.coordinates:
            if (
                axis in expected_instance.coordinates
                and instance.coordinates[axis] != expected_instance.coordinates[axis]
            ):
                yield FAIL, Message(
                    "bad-coordinate",
...
def com_google_fonts_check_repo_sample_image(readme_contents, readme_directory, config):
    ...
    for (dirpath, dirnames, filenames) in os.walk(readme_directory):
        local_image_files.extend(
            [
                os.path.join(dirpath[len(readme_directory) + 1 :], filename)
...
def com_google_fonts_check_metadata_can_render_samples(ttFont, family_metadata):
    ...
    if not family_metadata.sample_glyphs:
        passed = False
        yield INFO, Message("no-samples", "No sample_glyphs on METADATA.pb")
    else:
...
def com_google_fonts_check_metadata_category_hint(family_metadata):
    ...
    if (
        ...
        and not family_metadata.category == inferred_category
    ):
        yield WARN, Message(
            "inferred-category",

...
def com_google_fonts_check_monospace(ttFont, glyph_metrics_stats):
    ...
    for key in required:
        if key not in ttFont:
            missing_tables = True
            yield FAIL, Message(f"lacks-{key}", f"Font file lacks a '{key}' table.")
...
def com_google_fonts_check_name_match_familyname_fullfont(ttFont):
    ...
    if len(familyname) == 0:
        yield FAIL, Message(
            "no-font-family-name",
            "Font lacks a NameID.FONT_FAMILY_NAME" " entry in the 'name' table.",
        )
    elif len(fullfontname) == 0:
        yield FAIL, Message(
            "no-full-font-name",
            "Font lacks a NameID.FULL_FONT_NAME" " entry in the 'name' table.",
        )
    else:

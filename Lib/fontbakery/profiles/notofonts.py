    ...
    for cp, expected_width in expectations.items():
        ...
        if isinstance(expected_width, tuple):
            if got_width < math.floor(expected_width[0]) or got_width > math.ceil(
                ...
                problems.append(
                    f"0x{cp:02x} (got={got_width}, expected={expected_width[0]}...{expected_width[1]}"
                )
        else:
...
def com_google_fonts_check_cmap_alien_codepoints(ttFont, config):
    ...
    if pua:
        yield FAIL, Message(
            "pua-encoded",
            "The following private use area codepoints were encoded in the font: "
            + pretty_print_list(config, pua),
        )
    if surrogate:
        yield FAIL, Message(
            "surrogate-encoded",
            "The following surrogate pair codepoints were encoded in the font: "
            + pretty_print_list(config, surrogate),
        )

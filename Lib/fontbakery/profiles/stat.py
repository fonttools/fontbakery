...
@check(
    ...
    rationale = """
      According to the OpenType spec, there must be an Axis Record for every axis defined in the fvar table, https://docs.microsoft.com/en-us/typography/opentype/spec/stat#axis-records
    """,
    conditions = ['is_variable_font']
)
def com_google_fonts_check_varfont_stat_axis_record_for_each_axis(ttFont):
    """According to the OpenType spec, there must be an Axis Record for every
    axis defined in the fvar table."""
    ...
    if len(missing_axes) > 0:
        yield FAIL, Message("missing-axis-records", (
            "STAT table is missing Axis Records for the following "
            "axes {missing_axes}")
        )
    else:
